...
register_wallet() {
	btcli subnet register --wallet.name $1 --netuid 1 --wallet.hotkey default --subtensor.chain_endpoint $NETWORK --no_prompt
}
...
register() {
    ...
    echo "LOCALNET_SETUP - Listing some stats"
    btcli subnet lock_cost 2>&1 &
